...
describe('UserService', () => {
    ...
    describe('getById', () => {
        it('should return a user by ID', async () => {
            ...
            const [id] = await db('usersTable').insert({ name: 'John Doe', email: 'john@example.com', password: 'password' });
            ...
            expect(user === null || user === void 0 ? void 0 : user.password).toBeUndefined();
        });
    ...
    describe('create', () => {
        ...
        it('should throw a ConflictError for an existing email address', async () => {
            const existingUser = { name: 'John Doe', email: 'john@example.com', password: 'password' };
            await db('usersTable').insert(existingUser);
            const newUser = { name: 'Jane Doe', email: 'john@example.com', password: 'password' };
    ...
    describe('login', () => {
        it('should log in a user with correct credentials', async () => {
            const existingUser = { name: 'John Doe', email: 'john@example.com', password: 'password' };
            await db('usersTable').insert(Object.assign(Object.assign({}, existingUser), { password: (0, crypto_js_1.SHA256)(existingUser.password).toString() }));
            const loggedInUser = await userService.login('john@example.com', 'password');
